...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//                                                                                                                    //
// packages/tap_i18n/lib/tap_i18n/tap_i18n-client.coffee.js                                                           //
//                                                                                                                    //
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
                                                                                                                      //
__coffeescriptShare = typeof __coffeescriptShare === 'object' ? __coffeescriptShare : {}; var share = __coffeescriptShare;
var indexOf = [].indexOf || function(item) { for (var i = 0, l = this.length; i < l; i++) { if (i in this && this[i] === item) return i; } return -1; },
  slice = [].slice;                                                                                                   //
                                                                                                                      //
_.extend(TAPi18n.prototype, {                                                                                         // 1
  _languageSpecificTranslators: null,                                                                                 // 2
  _languageSpecificTranslatorsTrackers: null,                                                                         // 2
  _getLanguageFilePath: function(lang_tag) {                                                                          // 2
    var path;                                                                                                         // 6
    if (!this._enabled()) {                                                                                           // 6
      return null;                                                                                                    // 7
    }                                                                                                                 //
    path = this.conf.cdn_path != null ? this.conf.cdn_path : this.conf.i18n_files_route;                              // 6
    path = path.replace(/\/$/, "");                                                                                   // 6
    if (Meteor.isCordova && path[0] === "/") {                                                                        // 11
      path = Meteor.absoluteUrl().replace(/\/+$/, "") + path;                                                         // 12
    }                                                                                                                 //
    return path + "/" + lang_tag + ".json";                                                                           //
  },                                                                                                                  //
  _loadLanguage: function(languageTag) {                                                                              // 2
    var dependencyLoadDfd, dfd, directDependencyLanguageTag, loadLanguageTag, project_languages;                      // 39
    dfd = new $.Deferred();                                                                                           // 39
    if (!this._enabled()) {                                                                                           // 41
      return dfd.reject("tap-i18n is not enabled in the project level, check tap-i18n README");                       // 42
    }                                                                                                                 //
    project_languages = this._getProjectLanguages();                                                                  // 39
    if (indexOf.call(project_languages, languageTag) >= 0) {                                                          // 46
      if (indexOf.call(this._loaded_languages, languageTag) < 0) {                                                    // 47
        loadLanguageTag = (function(_this) {                                                                          // 48
          return function() {                                                                                         //
            var jqXHR;                                                                                                // 49
            jqXHR = $.getJSON(_this._getLanguageFilePath(languageTag));                                               // 49
            jqXHR.done(function(data) {                                                                               // 49
              _this._loadLangFileObject(languageTag, data);                                                           // 52
              _this._loaded_languages.push(languageTag);                                                              // 52
              return dfd.resolve();                                                                                   //
            });                                                                                                       //
        ...
        if (languageTag !== fallback_language && indexOf.call(project_languages, directDependencyLanguageTag) >= 0) {
          dependencyLoadDfd = this._loadLanguage(directDependencyLanguageTag);                                        // 65
          dependencyLoadDfd.done((function(_this) {                                                                   // 65
            return function() {                                                                                       //
              return loadLanguageTag();                                                                               //
            };                                                                                                        //
          })(this));                                                                                                  //
          dependencyLoadDfd.fail((function(_this) {                                                                   // 65
      ...
      dfd.reject(["Language " + languageTag + " is not supported"]);                                                  // 80
    }                                                                                                                 //
    return dfd.promise();                                                                                             // 82
  },                                                                                                                  //
  _registerHelpers: function(package_name, template) {                                                                // 2
    var helpers, tapI18nextProxy, underscore_helper;                                                                  // 85
    if (package_name !== globals.project_translations_domain) {                                                       // 85
      tapI18nextProxy = this._getPackageI18nextProxy(this.packages[package_name].namespace);                          // 86
    } else {                                                                                                          //
      tapI18nextProxy = this._getPackageI18nextProxy(globals.project_translations_domain);                            // 88
    }                                                                                                                 //
    underscore_helper = function() {                                                                                  // 85
      var args, key, options;                                                                                         // 91
      key = arguments[0], args = 2 <= arguments.length ? slice.call(arguments, 1) : [];                               // 91
      options = (args.pop()).hash;                                                                                    // 91
      if (!_.isEmpty(args)) {                                                                                         // 92
        options.sprintf = args;                                                                                       // 93
      }                                                                                                               //
      return tapI18nextProxy(key, options);                                                                           //
    };                                                                                                                //
    if (package_name !== globals.project_translations_domain) {                                                       // 98
      if ((Template[template] != null) && (Template[template].helpers != null)) {                                     // 100
        helpers = {};                                                                                                 // 101
        helpers[this.packages[package_name].helper_name] = underscore_helper;                                         // 101
        return Template[template].helpers(helpers);                                                                   //
      }                                                                                                               //
    } else {                                                                                                          //
      UI.registerHelper(this.conf.helper_name, underscore_helper);                                                    // 108
  ...
  _getRegisterHelpersProxy: function(package_name) {                                                                  // 2
  ...
  _prepareLanguageSpecificTranslator: function(lang_tag) {                                                            // 2
    var dfd;                                                                                                          // 119
    dfd = (new $.Deferred()).resolve().promise();                                                                     // 119
    if (lang_tag in this._languageSpecificTranslatorsTrackers) {                                                      // 121
      return dfd;                                                                                                     // 122
    }                                                                                                                 //
    this._languageSpecificTranslatorsTrackers[lang_tag] = new Tracker.Dependency;                                     // 119
    if (!(lang_tag in this._languageSpecificTranslators)) {                                                           // 126
      dfd = this._loadLanguage(lang_tag).done((function(_this) {                                                      // 127
        return function() {                                                                                           //
          _this._languageSpecificTranslators[lang_tag] = _this._getSpecificLangTranslator(lang_tag);                  // 129
    ...
    return (function(_this) {                                                                                         //
      return function(key, options, lang_tag) {                                                                       //
        ...
        if (((options != null ? options.lang : void 0) != null) && ((options != null ? options.lng : void 0) == null)) {
          options.lng = options.lang;                                                                                 // 142
        }                                                                                                             //
        if (((options != null ? options.lng : void 0) != null) && (lang_tag == null)) {                               // 144
          lang_tag = options.lng;                                                                                     // 145
          delete options.lng;                                                                                         // 145
        }                                                                                                             //
        if (lang_tag != null) {                                                                                       // 155
          _this._prepareLanguageSpecificTranslator(lang_tag);                                                         // 156
          _this._languageSpecificTranslatorsTrackers[lang_tag].depend();                                              // 156
          if (lang_tag in _this._languageSpecificTranslators) {                                                       // 160
            return _this._languageSpecificTranslators[lang_tag]((TAPi18n._getPackageDomain(package_name)) + ":" + key, options);
          } else {                                                                                                    //
            return TAPi18next.t((TAPi18n._getPackageDomain(package_name)) + ":" + key, options);                      // 163
          }                                                                                                           //
        }                                                                                                             //
        _this._language_changed_tracker.depend();                                                                     // 141
  ...
  setLanguage: function(lang_tag) {                                                                                   // 2
    var self;                                                                                                         // 175
    self = this;                                                                                                      // 175
    return this._loadLanguage(lang_tag).then((function(_this) {                                                       //
      return function() {                                                                                             //
        TAPi18next.setLng(lang_tag);                                                                                  // 178
        _this._language_changed_tracker.changed();                                                                    // 178
        return Session.set(_this._loaded_lang_session_key, lang_tag);                                                 //
      };                                                                                                              //
    })(this));                                                                                                        //
  },                                                                                                                  //
  getLanguage: function() {                                                                                           // 2
    var session_lang;                                                                                                 // 184
    if (!this._enabled()) {                                                                                           // 184
      return null;                                                                                                    // 185
    }                                                                                                                 //
    session_lang = Session.get(this._loaded_lang_session_key);                                                        // 184
    if (session_lang != null) {                                                                                       // 189
      return session_lang;                                                                                            //
    } else {                                                                                                          //
